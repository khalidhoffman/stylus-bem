...
Lexer.prototype = {
  ...
  advance: function() {
    ...
    if (this.shouldParseBemSelector && tok.val && tok.val.filename == this.bemBlockFilename) {
        var newStr = tok.val.string;
        if (currentBemBlock && newStr){
          if (tok.val.string.match(/^\_\_?/)){
            newStr = tok.val.string.replace(/^\_\_?/, '.' + currentBemBlock.val.string + '__');
            tok.val.string = newStr;
            tok.val.name = newStr;
          } else if (tok.val.string.match(/^\-\-?/)){
            newStr = tok.val.string.replace(/^\-\-?/, '&--');
            tok.val.string = newStr;
            tok.val.name = newStr;
          }
        }
    } else if (this.shouldParseBemBlock && tok.val && tok.val.string && tok.val.string.match(/^\w/)){
      this.bemBlocks.push(tok);
      this.bemBlockFilename = tok.val.filename;
    }
    this.shouldParseBemSelector = (currentBemBlock && tok.val == '/');
    this.shouldParseBemBlock = (tok.val == '.' && this.prev == 'newline');
    if(this.shouldParseBemSelector){
      return this.advance();
    }
    return tok;
  },
