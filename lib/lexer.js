...
Lexer.prototype = {
  ...
  advance: function() {
    ...
    var lastBemBlock = this.bemBlocks[this.bemBlocks.length - 1];

    if (tok.val && tok.val.string){
        var tokStr = tok.val.string;
        if(this.shouldParseBemBlock || this.shouldParseBemMod || this.shouldParseBemElement){
          // pop lastBlock because it is out of scope
          // we only check for pop when we're going to a modification
          // to prevent multiple pops over whitespace
          while(lastBemBlock && column <= (lastBemBlock.column+1)){
              var popped = this.bemBlocks.pop();
              lastBemBlock = this.bemBlocks[this.bemBlocks.length - 1];
          }
        }
        if (this.shouldParseBemBlock && tokStr.match(/^[a-zA-Z]/)){
          // push a bem block
          this.bemBlocks.push(tok);
          this.bemBlockFilename = tok.val.filename;

        } else if (this.shouldParseBemElement && lastBemBlock ){
        // modify bem selectors
          tokStr = tok.val.string.replace(/^\_\_?/, '.' + lastBemBlock.val.string + '__');
          tok.val.name = tokStr;
          tok.val.string = tokStr;
        } else if (this.shouldParseBemMod && lastBemBlock){
          tokStr = tok.val.string.replace(/^\-\-?/, '.' + lastBemBlock.val.string + '--');
          tok.val.name = tokStr;
          tok.val.string = tokStr;
        }
    } 

    // set bem selector flags
    if(lastBemBlock && tok.val == '/'){
      this.shouldParseBemMod = this.str[0] == '-';
      this.shouldParseBemElement = this.str[0] == '_';
    } else {
      this.shouldParseBemMod = false;
      this.shouldParseBemElement = false;
    }

    // if (this.str.match(/^\/\/PRINT/)) console.log('this: %s', require('util').inspect(this.bemBlocks));
    if(this.shouldParseBemMod || this.shouldParseBemElement){
      return this.advance();
    }


    // set bem block flag
    this.shouldParseBemBlock = ((this.prev === undefined || /outdent|indent|newline/.test(this.prev)) && tok.val == '.');
    if(this.shouldParseBemBlock){
