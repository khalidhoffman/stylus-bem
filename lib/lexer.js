...
Lexer.prototype = {
  ...
  advance: function() {
    ...
    if (tok.val && tok.val.filename == this.bemBlockFilename && lastBemBlock && tok.val.string) {
        // modify bem selectors
        var newStr = tok.val.string;
        if (this.shouldParseBemElement){
          newStr = tok.val.string.replace(/^\_\_?/, '.' + lastBemBlock.val.string + '__');
          tok.val.name = newStr;
          tok.val.string = newStr;
        } else if (this.shouldParseBemMod){
          newStr = tok.val.string.replace(/^\-\-?/, '.' + lastBemBlock.val.string + '--');
          tok.val.name = newStr;
          tok.val.string = newStr;
        }
    } else if (this.shouldParseBemBlock && tok.val && tok.val.string && tok.val.string.match(/^[a-zA-Z]/)){
      // push a bem block
